...
! output variables
      ...
      real,dimension(lonf2s:lonf2e,latg2s:latg2e)         ::   &
          ...
          QBAF_AVG,QNFIRE_AVG,QBURN_AVG                             !Huilin add fire variable
      real,dimension(lonf2s:lonf2e,latg2s:latg2e)         ::   &
...
!     if (mod(kyr,10) .eq. 0.) then
      if (mod(kyr,1) .eq. 0.) then       !Huilin used to keep ssib.inp
...
!input:
        ...
        LIGHT(i,j), DP(i,j), AGR(i,j), GDP(i,j), PEATF(i,j),         &  !Huilin add Light, DP 
...
!      call records(2,1,qprec60_avg,"prec60_avg","60-day prec","-")
