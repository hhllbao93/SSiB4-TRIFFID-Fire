...
!TRIF output to GCM
         ...
         LEAF1,ROOT1,WOOD1,DCVEG1,LIT_C,LIT_C_T,                      &
         AGB_AVG,FEO_AVG,FES_AVG,FE1_AVG,FE2_AVG,FD_AVG,FCLI,FSAT,    & 
         BAF_AVG,NFIRE_AVG,BURN_AVG,BURN_FIRE,                        & ! Huilin add fire output
...
!----------------------------------------------------------------------
      ...
      real:: psurf,sig, zlat,zwind,light,dp,agr,gdp,peatf            !Huilin add light dp agr gdp peatf
      real::                                                         &
        SUNANG, RLWDOWN,PPL,PPC,TM,UMM1,VMM1,QM, EM,                 &
        rhoair,radn11,radn12,radn21,radn22,radnsum
      real:: PRECD,PREC60_AVG
      real,dimension(10)  :: PREC10                                !Huilin Jul 2019
      real,dimension(60)  :: PREC60                                !Huilin Jul 2019
...
!yliu09May2017
      ...
      real:: FCLI,FSAT                                 !Huilin add for peatfire
      real:: BAF_AVG,NFIRE_AVG,BURN_AVG                !Huilin add for fire
...
!--------------------------------------------------------
! Huilin read PREC data for PREC10 PREC60
      PREC60              = dataios % xPREC60
      PRECD               = dataios % xPRECD
      PRECD               = PRECD+PPL+PPC
      if (mod(KREC,8).eq.0) then
         PRECD            = PRECD/8.0 
         DO DAY=1,9
            PREC10(DAY)   = dataios % xPREC10(DAY+1) 
         END DO
         PREC10(10)       = PRECD
         DO DAY=1,59
            PREC60(DAY)   = dataios % xPREC60(DAY+1) 
         END DO
         PREC60(60)       = PRECD
         dataios % xPRECD = 0.0 
      else 
         dataios % xPRECD = PRECD 
      end if
...
!
       ...
       DO N=1,10
         dataios % xPREC10(N)   = PREC10(N)
       END DO
       DO N=1,60
         dataios % xPREC60(N)   = PREC60(N)
       END DO
!
