...
! !CALLED FROM SFCTRIF:   
    ...
    real :: peatf                         !in peatland fraction 
...
!   
       agb_avg                  = agb_avg + (leaf1_fire(npft1)*phen(npft1) + wood1_fire(npft1)*0.5) * frac_trif(npft1)*1000.0                               !kg->g c/m^2
       fm_avg                   = fm_avg + ((1.0-max(0.0,min(1.0,(www2n(npft1)/0.42 &
                                  -fm_low(npft1))/(fm_up(npft1)-fm_low(npft1)))))**0.7*frac_trif(npft1))        !huilin add to test soil www2 on fire
       if (dp .gt. 0.1) then
...
! ********Peat fire*************
! Peat fire is not used in the current version of fire model   !
...
!     fcli                      = max(0.0,min(1.0,(4.0-prec60_avg)/4.0))**2 
