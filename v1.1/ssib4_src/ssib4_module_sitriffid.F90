      ...
      if (N.eq.1) then
         if ((FRAC(3).GT.0.3).OR.(FRAC(4).GT.0.3).OR.((FRAC(3)+FRAC(4)).GT.0.5)) then
           if (FRAC(N).GT.0.10) then
             B(N) = PC_S(N)*SPACE(N)/C_VEG(N)-0.01
           endif
         endif
      endif
      if (N.eq.7) then
...
!----------------------------------------------------------------------    
        CALL VEGCARB (N,FORW,GAMMA,G_LEAF(N),NPP(N),RESP_W(N),  &
                      ...
                      DLEAF(N),DROOT(N),DWOOD(N),               & !Huilin add DLEAF, DROOT, DWOOD to build c pool NOV 10 
                      DCVEG(N),PC_S(N))                                
...
!                                 !       (kg C/m2/360days).               
       ...
       ROOT,                    & ! INOUT Root biomass (kg C/m2).          
       WOOD,                    & ! INOUT Woody biomass (kg C/m2).         
       DLEAF,                   & ! INOUT Leaf biomass change (kg C/m2) Huilin    
       DROOT,                   & ! INOUT Root biomass change (kg C/m2) Huilin    
       DWOOD,                   & ! INOUT Woody biomass change (kg C/m2) Huilin   
